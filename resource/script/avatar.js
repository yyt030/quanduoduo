$(function(){
	$('.body_wrap.avatar .load_wrap .coupon-image[data-fid]').each(function(){
		...
		item.html(
			'<a href="/user/profile?fid='+fid+'" title="编辑信息">'+
				'<img src="'+avatar+'" onerror="this.src=\'/static/images/noavatar_small.jpg\'" />'+
			'</a>'+
			'<div class="photo-intro">'+
				'<div class="popover_inner">'+
					'<a href="/user/message?fid='+fid+'" title="消息记录"><i class="icon-envelope-alt"></i></a>'+
					'<a href="/discount/getlist?fid='+fid+'" title="领券记录"><i class="icon-ticket"></i></a>'+

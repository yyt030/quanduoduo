...
function cascdeInit(v1,v2,v3) {
	...
	if (window.ActiveXObject) {
		provinces = xmlDoc.selectSingleNode("address").childNodes;
	}
	else {
		provinces = xmlDoc.childNodes[0].getElementsByTagName("province");
	} 
    if (provinces.length > 0) {
        ...
        for (var i = 0; i < provinces.length; i++) {
            ...
            option.value = provinces[i].getAttribute("name");
            option.text = provinces[i].getAttribute("name");
            if (v1 == option.value) {
		...
		//城市列表
		citys = provinces[provinceindex].getElementsByTagName("city");
		if (citys.length> 0) {
			for (var i = 0; i < citys.length; i++) {
				//默认为第一个省份的城市
				var option = document.createElement("option");
				option.value = citys[i] .getAttribute("name");
				option.text = citys[i].getAttribute("name");
				if (v2 == option.value) {
					option.selected = true;
					cityindex = i;
				}
			...
			dists = citys[cityindex].getElementsByTagName("county");
			if (dists.length> 0) {
				//县区列表
				for (var i = 0; i < dists.length; i++) {
					var option = document.createElement("option");
					option.value = dists[i].getAttribute("name");
					option.text = dists[i].getAttribute("name");
					if (v3 == option.value) {
						option.selected = true;
					}
...
function selectCity() {
    ...
    countyNodes = provinces[dropElement1.selectedIndex];      
...
function selectcounty() {
    ...
    var city = document.getElementById("sel-area");  
    var countys = provinces[dropElement1.selectedIndex].getElementsByTagName("city")[dropElement2.selectedIndex].getElementsByTagName("county")
    RemoveDropDownList(city);
